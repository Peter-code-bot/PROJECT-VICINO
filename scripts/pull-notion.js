...
// Main function
async function main() {
  const OBSIDIAN_ROOT = path.resolve(__dirname, '../../VICINO OBSIDIAN/VICINO');
  if (!fs.existsSync(OBSIDIAN_ROOT)) {
    console.error(`Error: Local Obsidian directory not found at ${OBSIDIAN_ROOT}`);
    process.exit(1);
  }
  ...
  function readDirRecursive(dir) {
    ...
    for (const item of items) {
      if (item.startsWith('.')) continue; // skip .obsidian
  ...
  readDirRecursive(OBSIDIAN_ROOT);
  ...
  for (const doc of localDocs) {
    ...
    try {
      ...
      if (normLocal === normNotion) {
        ...
        console.log(`DISCREPANCY: Page '${doc.title}' content differs. Updating local file...`);
        fs.writeFileSync(doc.fullPath, `# ${doc.title}\n\n${notionMd}`, 'utf-8');
        console.log(`  ✅ Successfully updated ${doc.file}`);
      }
    } catch (e) {
