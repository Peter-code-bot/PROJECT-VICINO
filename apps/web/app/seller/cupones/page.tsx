    ...
    <div className="space-y-6 min-w-0">
      ...
      {coupons && coupons.length > 0 ? (
        <div className="space-y-3">
          {coupons.map((c) => {
            // `activo` admite NULL en la base (BOOLEAN DEFAULT TRUE, sin NOT
            // NULL). El nulo NO es "activo": la policy publica filtra con
            // `activo = TRUE`, que con NULL da NULL, asi que un cupon en ese
            // estado ya no lo ve ningun comprador. Lo normalizamos una sola vez
            // para que el badge y el toggle de CouponActions usen el mismo
            // criterio y el vendedor no active lo que cree estar apagando.
              ...
              <div key={c.id} className="rounded-[var(--r-xl)] bg-[color:var(--sidebar-bg)] p-4 space-y-2 min-w-0 overflow-hidden">
                <div className="flex items-center justify-between gap-3 min-w-0">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <span
                      className={`text-xs px-2 py-0.5 rounded-[var(--r-pill)] font-medium shrink-0 ${activo ? "bg-[color:var(--fg)] text-[color:var(--bg)]" : "bg-[color:var(--bg-elev-2)] text-[color:var(--fg-dim)] border border-[color:var(--border)]"}`}
                    >
                      {activo ? "Activo" : "Inactivo"}
                    </span>
                  </div>
                  <CouponActions id={c.id} activo={activo} />
                </div>
                <p className="text-sm">
