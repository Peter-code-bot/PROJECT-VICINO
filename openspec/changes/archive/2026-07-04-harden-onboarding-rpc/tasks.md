...
- [x] T-05 -- PARA y reportar a Pedro. Esperar sign-off antes de FASE B.

---

## FASE B -- SQL + commit (Claude, after Pedro signs off on FASE A)

- [x] T-06 -- studio-script.sql (4 blocks Camino 2):
...
- [x] T-07 -- mirror migration `supabase/migrations/20260704000001_harden_complete_user_onboarding.sql`
  (idempotent `CREATE OR REPLACE` + REVOKE/GRANT; pattern `20260521000011_rpc_update_profile_and_pause.sql`).
- [x] T-08 -- confirm NO app edits required (client already wired); the only allowed edit is
...
- [x] T-10 -- CODEX adversarial review loop (CLAUDE.md) on the SQL (migrations = max priority):
...
- [x] P-1 -- Open Studio SQL editor for project `oxxdkwywprkfghhbnoto`.
- [x] P-2 -- Paste BLOCK 1 (SNAPSHOT), run. Capture current `prosecdef`/`proconfig`/grants of
  the live RPC (confirms whether it was DEFINER + anon-executable as suspected).
- [x] P-3 -- Paste BLOCK 2 (DRY-RUN), run. Confirms the WRITE applies cleanly; ROLLBACK
  persists nothing.
- [x] P-4 -- Paste BLOCK 3 (APPLY), run. Confirm COMMIT, no errors.
- [x] P-5 -- Paste ledger INSERT (bookkeeping note above), run.
- [x] P-6 -- Paste BLOCK 4 (VERIFY), run. Expected: `prosecdef=true` (DEFINER); `proconfig`
  contains `search_path=""`; `authenticated` has EXECUTE; `anon`/`PUBLIC` have none.
- [x] P-7 -- RLS smoke test (fill `<UUID>` with a real test user): `has_seen_onboarding`
...
- [x] T-11 -- Client verification (2 viewports, mobile 375x812 + desktop 1280x800): fresh
...
- [x] T-12 -- `pnpm type-check` green.
- [x] T-13 -- push (PAT; clean remote URL after) + open PR to master. Wait for Pedro's
  validation before FF-only merge.
- [x] T-14 -- After merge: `openspec archive` this change; delete local + remote branch;
